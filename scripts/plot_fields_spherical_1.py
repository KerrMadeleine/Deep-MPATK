...
for m in range(len(STAGYY_MODS)):
    ...
    print('making plots for:', STAGYY_MODS[m])

    test_model.set_rprof_dictionary(rprof_n0,rprof_n1)

    time = test_model.rprof_data_dict['time'][:,0]
    rad = test_model.rprof_data_dict['r'][0] + test_model.Rc

    print(rad)


    for ts in range(0,test_model.nmax+1,100):

        T_table = test_model.getTableT(ts)
        # print(T_table)
        print(test_model.getRadialprofile(T_table,0))
        print(test_model.getHorizontalprofile(T_table,0))
        V_table = test_model.getTableV(ts)
        #print(V_table)

        print(test_model.getMeanRadialprofile(T_table))

        fig, ax = plt.subplots(1)
        fig.set_size_inches(9.5,7)
        plt.plot(test_model.getMeanRadialprofile(T_table), rad)
        plt.plot(test_model.getTrimmedMeanRadialprofile(T_table), rad)
        plt.show()
        plt.clf()
        plt.close()
        ...
        fig, ax = plt.subplots(1)
        fig.set_size_inches(9.5,7)
        plt.plot(test_model.y, test_model.getMeanHorizontalprofile(T_table))
        plt.plot(test_model.y, test_model.getTrimmedMeanHorizontalprofile(T_table))
        plt.show()
        plt.clf()
        plt.close()

        #test_model.plotT_2Dcart(T_table)
        #test_model.plotlogV_2Dcart(V_table)

        fig, ax = plt.subplots(1,subplot_kw={'projection': 'polar'})
        fig.set_size_inches(9.5,7)
        fig.suptitle('T at t='+ str(time[ts]) +' Myr',size=20)
        XCMB,YDEP = np.meshgrid(test_model.y,rad)
        c=ax.pcolor(XCMB,YDEP,T_table,cmap=cm.lajolla)#,vmin=1000,vmax=3500)
        ax.set_rticks([3110e3,3110e3+test_model.D])  # Less radial ticks
        ax.set_thetagrids([])
